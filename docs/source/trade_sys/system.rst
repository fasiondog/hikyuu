...
:class:`MultiSystem` 支持两种运行模式（``set_mode``）：

- **模式 A（信号汇总，默认）**：父给每个子系统一个固定「影子账户」（虚拟资金与父解耦），子系统作为纯信号源；父通过 MM L1/L2 按权重（默认等权）换算数量并统一下单。
- **模式 B（资金划拨 / FOF-MOM）**：父给每个子系统「由上层分配额度的真实账户」（初始额度 ``set_sub_init_cash``），子系统在自己额度内自主决策，父通过 MM L2 **透传**其真实指令；调仓日父回写下期额度（滞后一期，支持嵌套穿透）。

聚合回测还支持：

- **MM L1/L2/L3 分层**：L1 系统级分配（模式 A 名义权重 / 模式 B 真实额度）、L2 行为级换算（A 按比重换算 / B 透传）、L3 组合风控（集中度上限 ``max-single-position``，默认 1.0 不限制）。
...
公共参数：
    ...
    * **can_trade_when_high_eq_low=False** *(bool)* : 当最高价等于最低价时，是否允许交易（一字涨停不可买入、一字跌停不可卖出）
