...
.. important::
    ...
    服务由独立的 VIP 插件 ``shmserver`` 提供，需要有效的 VIP 授权；未安装插件或授权无效时
    :func:`start_shm_server` 返回 ``False``。
...
典型用法：在常驻的服务端进程里启动服务，其余研究 / 回测进程正常 ``import hikyuu`` 即自动以
客户端身份接入：
...
::
    ...
    # 进程 B / C / … —— 客户端
    import hikyuu as hku      # 若进程 A 已启动服务且 use_shm_server=True，本进程自动进入客户端模式
...
.. note::
    ...
    服务端进程只作快照发布者，并不需要接入其他 shm 服务。若在服务端进程内通过
    ``load_hikyuu`` 初始化，应传入 ``use_shm_server=False`` 以跳过初始化阶段的客户端探测；
    否则该进程会先当作客户端去连接既有服务，在重试约 10 秒后打印
    ``Failed connect to hikyuu shm server, fallback to standalone mode!`` 告警再降级为独立模式
    （数据仍能正常加载与发布，仅多出启动等待与一条无害告警）。命令行工具 ``shmserver`` 已默认如此。
...
服务端进程启动后，其余研究 / 回测进程照常 ``import hikyuu`` 即自动以客户端身份接入（需相同
``datadir`` 且 ``use_shm_server = True``）。
...
2. **客户端协商**：普通进程初始化时，若 ``use_shm_server`` 为真，则尝试连接既有服务；连接
   不上时按 500 毫秒间隔重试 20 次（覆盖服务端"正在启动"的窗口）。仍连不上则 **降级为独立模式**，
   自行加载全部数据——**绝不** 在本进程拉起服务。
...
5. **等待就绪**：客户端连接后轮询服务端的加载进度。服务端在基础数据（证券列表、市场、证券
   类型、节假日、板块等）加载完毕即对外提供服务，**不等待 K 线预加载完成**；客户端的等待上限
   由 ``shm_server_wait_timeout`` 控制，超时后同样降级为独立模式。
...
在每个进程的 ``hikyuu.ini`` 中设置 ``use_shm_server = False``；或干脆不启动服务端。
