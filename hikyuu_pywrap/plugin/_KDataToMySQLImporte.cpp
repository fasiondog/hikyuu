...
void export_plugin_KDataToMySQLImporter(py::module& m) {
    ...
    The K-line data importer (the MySQL storage), used to write the K-line / time-line / tick data obtained from the external data sources to MySQL,
    and supports registering the new markets and the security types to the baseinfo database.

    The actual writing is done by the import2mysql plugin (a VIP plugin, requiring a valid device license). When the plugin is missing or the license is invalid, at the construction stage
    ...
    The data organization is consistent with the hikyuu MySQL data driver: with {market}_{ktype} as the database name and the security code as the table name
    (e.g. sh_day.600000); the time-line is written to {market}_time and the tick data to {market}_trans.
    ...
    Typical usage::
        ...
        >>> if not im.set_config('127.0.0.1', 3306, 'root', 'pwd'):  # returning False usually means there is no valid license
    ...
    Set the MySQL connection information; it must be called before the other interfaces.

    :param str host: the MySQL host address
    :param int port: the MySQL port
    :param str usr: the login user name
    :param str pwd: the login password
    :param str baseinfo_db: the baseinfo database name; add_market and add_stock_type write to this database; it must be consistent with the baseinfo database used in the hikyuu configuration
    ...
    Append and write the time-line data for the specified market and the specified security (written to the {market}_time database, with the security code as the table name).
    ...
    Append and write the tick data for the specified market and the specified security (written to the {market}_trans database, with the security code as the table name).
