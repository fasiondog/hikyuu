...
void export_factor_main(py::module& m) {
    ...
    m.def("save_factorset", &saveFactorSet, py::arg("set"), "保存因子集");
    m.def("get_factorset", &getFactorSet, py::arg("name"), py::arg("ktype") = KQuery::DAY,
          "获取因子集");
    m.def("remove_factorset", &removeFactorSet, py::arg("name"), py::arg("ktype"), "删除因子集");
    m.def("get_all_factorsets", &getAllFactorSets, "获取所有因子集");
}
