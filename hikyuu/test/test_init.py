...
def get_real_tdx_filepath(code, market='sh'):
    ...
    2. 如果未设置, 则检查 /mnt/d1581/tdx/vipdoc/ 是否存在, 如果存在, 则使用该路径。
    3. 如果上述路径不存在, 则检查 test_data/vipdoc/ 是否存在, 如果存在, 则使用该路径。
    4. 如果上述路径都不存在, 则自动回退到在项目根目录下的 `test_data` 文件夹中寻找同名文件作为备用。
    ...
    # 检查 /mnt/d1581/tdx/vipdoc/
    base_path = "/mnt/d1581/tdx/vipdoc"
