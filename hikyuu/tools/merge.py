...
def merge_all_data_from_split(src_dir, dest_dir, data_types, stop_flag=None, progress_callback=None):
    """
    从 split 工具的拆分目录合并数据
...
class MergeApp:
    ...
    def _process_merge(self, data_types):
        ...
        try:
            ...
            if self.is_processing:  # 正常完成
                if success:
                    ...
                    self.root.after(0, lambda: messagebox.showwarning("提示", "未找到需要合并的数据文件！\n\n请确认：\n1. 源目录选择正确（split 拆分后的目录）\n2. 数据类型勾选正确\n3. 年份文件命名格式为：{market}_{type}_{YYYY}.h5"))
            else:  # 用户取消
    ...
    def _start_merge(self):
        ...
        # 在新线程中执行
        thread = threading.Thread(target=self._process_merge, args=(data_types,), daemon=True)
        thread.start()
