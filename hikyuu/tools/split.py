...
def _split_data_by_year(src_file_name, dest_dir, data_type, table_desc, stop_flag=None, progress_callback=None):
    ...
    try:
        ...
        # 注意：这里存储的应该是实际需要处理的年份范围（已考虑增量边界）
                ...
                for src_table in stock_tables_iter_inner:
                    ...
                    if stock_name in stock_year_ranges:
                        ...
                        if stock_max_year < year or stock_min_year > year:
                            # ✓ 修复：只跳过今年，不要修改 stock_current_index
                            # 因为 stock_year_ranges 存储的是调整后的年份范围（已考虑增量边界）
                            # 不代表源表的实际年份范围，后续年份可能还有数据
