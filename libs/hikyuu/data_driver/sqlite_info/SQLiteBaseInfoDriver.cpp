...
loadMarketInfo(map<string, MarketInfo>& out) {
    if (!m_db) {
        return false;
    }
...
int SQLiteBaseInfoDriver::_getMarketTableCallBack(void *out,
    ...
    try{
        ...
        (*(map<string, MarketInfo> *)out)[market] = marketInfo;
        result = 0;
...
bool SQLiteBaseInfoDriver::loadStockTypeInfo(
        map<hku_uint32, StockTypeInfo>& out) {
    if (!m_db) {
        return false;
    }
...
int SQLiteBaseInfoDriver::_getStockTypeInfoTableCallBack(
    ...
    try{
        ...
        (*((map<hku_uint32, StockTypeInfo> *)out))[type] = stockTypeInfo;
        result = 0;
    }catch(boost::bad_lexical_cast& e){
