...
void OrderBrokerBase::buy(Datetime datetime, const string& market, const string& code,
                          ...
                          SystemPart from, const string& remark) noexcept {
    try {
        _buy(datetime, market, code, price, num, stoploss, goalPrice, from, remark);
...
void OrderBrokerBase::sell(Datetime datetime, const string& market, const string& code,
                           ...
                           SystemPart from, const string& remark) noexcept {
    try {
        _sell(datetime, market, code, price, num, stoploss, goalPrice, from, remark);
    } catch (const std::exception& e) {
        HKU_ERROR(e.what());
    } catch (...) {
        HKU_ERROR_UNKNOWN;
