...
public:
    ...
    : m_done(false),
      ...
      m_exit_thread_callback(exit_thread_callback) {
        try {
            m_threads_status.resize(m_worker_num, nullptr);
...
private:
    ...
    std::vector<bool*> m_threads_status;           // 工作线程状态
    ...
    static thread_local bool m_thread_need_stop;             // 线程停止运行指示
#endif
