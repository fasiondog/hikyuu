...
BOOST_AUTO_TEST_CASE( test_Stock_getWeight ) {
    ...
    BOOST_CHECK(weightList.size() == 23);
    weight = weightList.front();
    BOOST_CHECK(weight.datetime() == Datetime(199104030000L));
    BOOST_CHECK(weight.countAsGift() == 0.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    BOOST_CHECK(weight.priceForSell() == 0.0);
    BOOST_CHECK(weight.bonus() == 0.0);
    BOOST_CHECK(weight.increasement() == 0.0);
    BOOST_CHECK(weight.totalCount() == 150);
    BOOST_CHECK(weight.freeCount() == 68);

    weight = weightList.back();
    BOOST_CHECK(weight.datetime() == Datetime(201108050000L));
    BOOST_CHECK(weight.countAsGift() == 0.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    BOOST_CHECK(weight.priceForSell() == 0.0);
    BOOST_CHECK(weight.bonus() == 0.0);
    ...
    weightList = stock.getWeight(Datetime(199501010000),
                                  Datetime(199701010000));
    BOOST_CHECK(weightList.size() == 2);
    weight = weightList.front();
    BOOST_CHECK(weight.datetime() == Datetime(199509250000));
    BOOST_CHECK(weight.countAsGift() == 2.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    BOOST_CHECK(weight.priceForSell() == 0.0);
    BOOST_CHECK(weight.bonus() == 3.0);
    BOOST_CHECK(weight.increasement() == 0.0);
    BOOST_CHECK(weight.totalCount() == 51728);
    BOOST_CHECK(weight.freeCount() == 35721);

    weight = weightList.back();
    BOOST_CHECK(weight.datetime() == Datetime(199605270000));
    BOOST_CHECK(weight.countAsGift() == 5.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    BOOST_CHECK(weight.priceForSell() == 0.0);
    BOOST_CHECK(weight.bonus() == 0.0);
    BOOST_CHECK(weight.increasement() == 5.0);
    BOOST_CHECK(weight.totalCount() == 103456);
    BOOST_CHECK(weight.freeCount() == 71393);

    /** @arg 查询指定日期范围内的权息信息，指定的起始日期刚好存在对应权息记录，而结束日期没有 */
    weightList = stock.getWeight(Datetime(199509250000),
                                  Datetime(199701010000));
    BOOST_CHECK(weightList.size() == 2);
    weight = weightList.front();
    BOOST_CHECK(weight.datetime() == Datetime(199509250000));
    ...
    weightList = stock.getWeight(Datetime(199501010000),
                                  Datetime(199605270000));
    BOOST_CHECK(weightList.size() == 1);
    weight = weightList.front();
    BOOST_CHECK(weight.datetime() == Datetime(199509250000));
    BOOST_CHECK(weight.countAsGift() == 2.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    ...
    BOOST_CHECK(weightList.size() == 1);
    weight = weightList.front();
    BOOST_CHECK(weight.datetime() == Datetime(201108050000));
    BOOST_CHECK(weight.countAsGift() == 0.0);
    BOOST_CHECK(weight.countForSell() == 0.0);
    BOOST_CHECK(weight.priceForSell() == 0.0);
    BOOST_CHECK(weight.bonus() == 0.0);
...
BOOST_AUTO_TEST_CASE( test_Stock_getCount ) {
    ...
    BOOST_CHECK(stock.getCount(KQuery::DAY) == 5121);

    /** @arg 查询sz000001的日线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::DAY) == 4937);

    /** @arg 查询sh000001的周线数量*/
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::WEEK) == 1059);

    /** @arg 查询sz000001的周线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::WEEK) == 1037);

    /** @arg 查询sh000001的月线数量*/
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MONTH) == 253);

    /** @arg 查询sz000001的月线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::MONTH) == 250);

    /** @arg 查询sh000001的季线数量*/
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::QUARTER) == 85);

    /** @arg 查询sz000001的季线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::QUARTER) == 84);

    /** @arg 查询sh000001的半年线数量*/
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::HALFYEAR) == 43);

    /** @arg 查询sz000001的半年线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::HALFYEAR) == 42);

    /** @arg 查询sh000001的年线数量*/
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::YEAR) == 22);

    /** @arg 查询sz000001的年线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::YEAR) == 21);

    /** @arg 查询sh000001的分钟线数量 */
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN) == 682823L);

    /** @arg 查询sz000001的分钟线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN) == 641091L);

    /** @arg 查询sh000001的5分钟线数量 */
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN5) == 133980L);

    /** @arg 查询sz000001的5分钟线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN5) == 126381L);

    /** @arg 查询sh000001的15分钟线数量 */
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN15) == 44750L);

    /** @arg 查询sz000001的15分钟线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN15) == 42195L);

    /** @arg 查询sh000001的30分钟线数量 */
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN30) == 22380L);

    /** @arg 查询sz000001的30分钟线数量 */
    stock = sm.getStock("Sz000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN30) == 21101L);

    /** @arg 查询sh000001的60分钟线数量 */
    stock = sm.getStock("sh000001");
    BOOST_CHECK(stock.getCount(KQuery::MIN60) == 11193L);
...
BOOST_AUTO_TEST_CASE( test_Stock_getIndexRange ) {
    ...
    /** @arg 日期方式选定正常范围，且开始/结束日期在数据中刚好有对应的数据。日线/周线/月/季/半年/年/1分钟/5分钟/15分钟/30分钟/60分钟 */
...
BOOST_AUTO_TEST_CASE( test_Stock_getKRecordList ) {
    ...
    BOOST_CHECK(result[0] == KRecord(Datetime(199012190000), 96.050, 99.980, 95.790, 99.980, 49.4, 1260));
    BOOST_CHECK(result[10] == KRecord(Datetime(199101030000), 128.840, 130.140, 128.840, 130.140, 9.3, 141));
    BOOST_CHECK(result[5120] == KRecord(Datetime(201112060000), 2326.660, 2331.892, 2310.155, 2325.905, 4262559.5, 45917078));
    ...
    BOOST_CHECK(result[0] == KRecord(Datetime(199012200000), 104.300, 104.390, 99.980, 104.390, 8.4, 197));
    BOOST_CHECK(result[1] == KRecord(Datetime(199012210000), 109.070, 109.130, 103.730, 109.130, 1.6, 28));
    BOOST_CHECK(result[8] == KRecord(Datetime(199101020000), 127.610, 128.840, 127.610, 128.840, 5.9, 91));
    ...
    result = stock.getKRecordList(total, total, KQuery::DAY);
    BOOST_CHECK(result.size() == 0);

    /** @arg 日线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::DAY);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == 1);
    BOOST_CHECK(result[0] == KRecord(Datetime(201112060000), 2326.660, 2331.892, 2310.155, 2325.905, 4262559.5, 45917078));
    ...
    result = stock.getKRecordList(total, total, KQuery::WEEK);
    BOOST_CHECK(result.size() == 0);

    /** @arg 周线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::WEEK);
    BOOST_CHECK(result.size() == 0);
    ...
    result = stock.getKRecordList(total, total, KQuery::MONTH);
    BOOST_CHECK(result.size() == 0);

    /** @arg 月线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MONTH);
    BOOST_CHECK(result.size() == 0);
    ...
    result = stock.getKRecordList(total, total, KQuery::QUARTER);
    BOOST_CHECK(result.size() == 0);

    /** @arg 季线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::QUARTER);
    BOOST_CHECK(result.size() == 0);
    ...
    result = stock.getKRecordList(total, total, KQuery::HALFYEAR);
    BOOST_CHECK(result.size() == 0);

    /** @arg 半年线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::HALFYEAR);
    BOOST_CHECK(result.size() == 0);
    ...
    result = stock.getKRecordList(total, total, KQuery::YEAR);
    BOOST_CHECK(result.size() == 0);

    /** @arg 年线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::YEAR);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == total);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001040931), 1366.58, 1368.692, 1366.579, 1368.692, 4124880, 191158));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001040932), 1368.3, 1368.491, 1368.3, 1368.491, 2783200, 106916));
    BOOST_CHECK(result[682822] == KRecord(Datetime(201112061500), 2325.38, 2326.1, 2324.6, 2325.900, 5215150, 566312));
    ...
    BOOST_CHECK(result.size() == 9);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001040932), 1368.3, 1368.491, 1368.3, 1368.491, 2783200, 106916));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001040933), 1368.859, 1369.474, 1368.859, 1369.308, 1060260, 37789));
    BOOST_CHECK(result[8] == KRecord(Datetime(200001040940), 1367.651, 1367.662, 1367.389, 1367.389, 1335200, 41924));
    ...
    result = stock.getKRecordList(total, total, KQuery::MIN);
    BOOST_CHECK(result.size() == 0);

    /** @arg 分钟线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MIN);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == 1);
    BOOST_CHECK(result[0] == KRecord(Datetime(201112061500), 2325.38, 2326.1, 2324.6, 2325.900, 5215150, 566312));
    ...
    BOOST_CHECK(result.size() == total);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001040935), 1366.58, 1369.68, 1366.579, 1369.187, 106611.5, 438171));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001040940), 1369.187, 1369.371, 1367.389, 1367.389, 70687.3, 251473));
    BOOST_CHECK(result[133979] == KRecord(Datetime(201112061500), 2323.059, 2325.931, 2323.059, 2325.905, 176347.1, 1873675));
    ...
    BOOST_CHECK(result.size() == 9);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001040940), 1369.187, 1369.371, 1367.389, 1367.389, 70687.3, 251473));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001040945), 1367.389, 1367.389, 1364.645, 1364.763, 73577.1, 223346));
    BOOST_CHECK(result[8] == KRecord(Datetime(200001041020), 1366.043, 1366.458, 1365.946, 1366.458, 111601.7, 205190));
    ...
    result = stock.getKRecordList(total, total, KQuery::MIN5);
    BOOST_CHECK(result.size() == 0);

    /** @arg 5分钟线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MIN5);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == 1);
    BOOST_CHECK(result[0] == KRecord(Datetime(201112061500), 2323.059, 2325.931, 2323.059, 2325.905, 176347.1, 1873675));
    ...
    BOOST_CHECK(result.size() == total);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001040945), 1366.58, 1369.68, 1364.645, 1364.763, 250875.9, 912990));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041000), 1364.763, 1364.763, 1361.459, 1361.459, 204277.1, 457452));
    BOOST_CHECK(result[44749] == KRecord(Datetime(201112061500), 2323.068, 2325.931, 2320.819, 2325.905, 402339.3, 4060591));
    ...
    BOOST_CHECK(result.size() == 9);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001041000), 1364.763, 1364.763, 1361.459, 1361.459, 204277.1, 457452));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041015), 1361.558, 1366.093, 1361.295, 1365.927, 307531.8, 548313));
    BOOST_CHECK(result[8] == KRecord(Datetime(200001041330), 1399.56, 1399.664, 1396.679, 1399.325, 407799.9, 554965));
    ...
    result = stock.getKRecordList(total, total, KQuery::MIN15);
    BOOST_CHECK(result.size() == 0);

    /** @arg 15分钟线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MIN15);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == 1);
    BOOST_CHECK(result[0] == KRecord(Datetime(201112061500), 2323.068, 2325.931, 2320.819, 2325.905, 402339.3, 4060591));
    ...
    BOOST_CHECK(result.size() == total);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001041000), 1366.58, 1369.68, 1361.459, 1361.459, 455153, 1370442));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041030), 1361.558, 1367.683, 1361.295, 1367.683, 586634.5, 1000294));
    BOOST_CHECK(result[22379] == KRecord(Datetime(201112061500), 2327.919, 2327.919, 2320.819, 2325.905, 660297.5, 6415054));
    ...
    BOOST_CHECK(result.size() == 9);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001041030), 1361.558, 1367.683, 1361.295, 1367.683, 586634.5, 1000294));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041100), 1367.667, 1378.543, 1367.6, 1377.098, 622250.5, 1248350));
    BOOST_CHECK(result[8] == KRecord(Datetime(200001051030), 1409.486, 1421.612, 1409.486, 1421.139, 1397089.8, 1777404));
    ...
    result = stock.getKRecordList(total, total, KQuery::MIN30);
    BOOST_CHECK(result.size() == 0);

    /** @arg 30分钟线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MIN30);
    BOOST_CHECK(result.size() == 0);
    ...
    BOOST_CHECK(result.size() == 1);
    BOOST_CHECK(result[0] == KRecord(Datetime(201112061500), 2327.919, 2327.919, 2320.819, 2325.905, 660297.5, 6415054));
    ...
    BOOST_CHECK(result.size() == total);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001041030), 1366.58, 1369.68, 1361.295, 1367.683, 1041787.5, 2370736));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041130), 1367.667, 1387.853, 1367.6, 1387.853, 1411191.4, 2172479));
    BOOST_CHECK(result[11192] == KRecord(Datetime(201112061500), 2326.036, 2331.378, 2320.819, 2325.905, 1332210, 11886356));
    ...
    BOOST_CHECK(result.size() == 9);
    BOOST_CHECK(result[0] == KRecord(Datetime(200001041130), 1367.667, 1387.853, 1367.6, 1387.853, 1411191.4, 2172479));
    BOOST_CHECK(result[1] == KRecord(Datetime(200001041400), 1389.996, 1403.996, 1389.97, 1400.685, 1740709.6, 2348753));
    BOOST_CHECK(result[8] == KRecord(Datetime(200001061130), 1420.098, 1420.098, 1410.706, 1411.387, 1291509.5, 1378193));
    ...
    result = stock.getKRecordList(total, total, KQuery::MIN60);
    BOOST_CHECK(result.size() == 0);

    /** @arg 60分钟线 start > end */
    result = stock.getKRecordList(2, 1, KQuery::MIN60);
    BOOST_CHECK(result.size() == 0);
...
BOOST_AUTO_TEST_CASE( test_Stock_getKRecordByDate ) {
    ...
    /** @arg 日期小于第一条记录 */
    record = stock.getKRecordByDate(Datetime(199012180000), KQuery::DAY);
    ...
    BOOST_CHECK(record == KRecord(Datetime(199502150000), 547.8300, 548.5900, 542.5000, 542.8400, 11889.3000, 232116.0000));
    ...
    /** @arg 日期小于第一条记录 */
    record = stock.getKRecordByDate(Datetime(199012160000), KQuery::WEEK);
    BOOST_CHECK(record == Null<KRecord>());
    ...
    /** @arg 日期小于第一条记录 */
    record = stock.getKRecordByDate(Datetime(199011300000), KQuery::MONTH);
    BOOST_CHECK(record == Null<KRecord>());
    ...
    /** @arg 日期小于第一条记录 */
    record = stock.getKRecordByDate(Datetime(200001040944), KQuery::MIN15);
    BOOST_CHECK(record == Null<KRecord>());

    /** @arg 日期等于第一条记录 */
    record = stock.getKRecordByDate(Datetime(200001040945), KQuery::MIN15);
    BOOST_CHECK(record == KRecord(Datetime(200001040945), 1366.5800, 1369.6800, 1364.6450, 1364.7630, 250875.9000, 912990.0000));

    /** @arg 中间存在数据的随机一条数据 */
    record = stock.getKRecordByDate(Datetime(200001071045), KQuery::MIN15);
    BOOST_CHECK(record == KRecord(Datetime(200001071045), 1482.5210, 1490.0070, 1481.9910, 1485.2770, 1030800.2000, 1663302.0000));
    ...
    BOOST_CHECK(record == KRecord(Datetime(201112061500), 2323.0680, 2325.9310, 2320.8190, 2325.9050, 402339.3000, 4060591.0000));
...
BOOST_AUTO_TEST_CASE( test_Stock_id_map ) {
    ...
    dict[x1] = 1;
    dict[x3] = 3;
    BOOST_CHECK(dict.count(x1) != 0);
    BOOST_CHECK(dict.count(x2) != 0);
    BOOST_CHECK(dict.count(x3) != 0);
    BOOST_CHECK(dict[x1] == 1);
    BOOST_CHECK(dict[x2] == 1);
