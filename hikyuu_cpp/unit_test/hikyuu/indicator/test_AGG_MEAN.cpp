...
/**
 * @defgroup test_indicator_AGG_MEAN test_indicator_AGG_MEAN
 * @ingroup test_hikyuu_indicator_suite
 * @{
 */

/** @par 检测点 */
TEST_CASE("test_AGG_MEAN") {
    ...
    /** @arg 单日分钟线聚合 */
    auto ind = AGG_MEAN(CLOSE(), KQuery::MIN);
    auto result = ind(k);
    CHECK_EQ(result.size(), k.size());
    CHECK_EQ(result.name(), "AGG_MEAN");
    ...
    CHECK_EQ(result[0], doctest::Approx(sum / mink2.size()));
    ...
    CHECK_EQ(result.back(), doctest::Approx(sum / mink2.size()));
    ...
    CHECK_EQ(result[3], doctest::Approx(sum / mink2.size()));
}
...
/** @par 检测点 */
TEST_CASE("test_AGG_MEAN_export") {
    HKU_IF_RETURN(!pluginValid(), void());

    StockManager& sm = StockManager::instance();
    string filename(sm.tmpdir());
    filename += "/AGG_MEAN.xml";

    Stock stock = sm.getStock("sh000001");
    KData kdata = stock.getKData(KQuery(-20));
    Indicator x1 = AGG_MEAN(CLOSE())(kdata);
    {
