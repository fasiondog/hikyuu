...
TEST_CASE("test_LruCache_thread_safety") {
    LruCache<int, int> cache(1000, 0);  // 增加容量以避免并发测试时的LRU淘汰，溢出容量为0
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {  // 移除ops_per_thread的显式捕获，因为它是通过&整体捕获的
    ...
    // 为了并发读写测试，使用更大的容量避免LRU淘汰
    cache.setOverflow(0);  // 设置溢出容量为0
    cache.resize(2000);  // 调整容量以避免LRU影响
    
    // 写线程
    for (int w = 0; w < 3; ++w) {
        rw_threads.emplace_back([w, &cache, &stop_flag]() {
            int key_offset = w * 1000;
            int key = key_offset;
            while (!stop_flag.load()) {
                cache.insert(key % 500, key * 3);  // 在小范围内循环使用键
                key++;
    ...
    // 读线程
    for (int r = 0; r < 3; ++r) {
        rw_threads.emplace_back([r, &cache, &stop_flag]() {
            int key_offset = r * 1000;
            int key = key_offset;
            while (!stop_flag.load()) {
                int val = cache.get(key % 500);
...
TEST_CASE("test_LruCache_overflow") {
    ...
    CHECK_EQ(cache.size(), 5);  // 仍应等于容量+溢出容量
    ...
    // 此时缓存大小为5，但容量+溢出容量变为4，应该触发清理
    CHECK_EQ(cache.size(), 4);  // 应该减少到容量+溢出容量
    ...
    cache2.resize(1);  // 调整容量为1，总容量变为1+1=2
    
    cache2.insert(4, "four");
    cache2.insert(5, "five");  // 超过新容量+溢出容量，应该触发清理
    
    CHECK_EQ(cache2.size(), 2);  // 应该等于新容量+溢出容量
}
